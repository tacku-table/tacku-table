  ...
  const onCheckedFood = useCallback(
    (checked: boolean, newItem: string) => {
      if (checked) {
        ...
        sessionStorage.setItem(
          "filteredFoodData",
          JSON.stringify(filteredFood.filter((ele) => ele !== newItem))
        );
  ...
  const onCheckedTime = useCallback(
    (checked: boolean, newItem: string) => {
      if (checked) {
        ...
        sessionStorage.setItem(
          "filteredTimeData",
          JSON.stringify(filteredTime.filter((ele) => ele !== newItem))
        );
      ...
      <div className="w-full flex justify-center items-center mb-[30px]">
        <button
          ...
          className={cls(
            ...
            !lastDoc || text || filteredFood?.length || filteredTime?.length
              ? "hidden"
              : ""
          )}
        >
