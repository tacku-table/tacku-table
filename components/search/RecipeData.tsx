...
const RecipeData = ({ dataResults, checkedList, checkedList2 }: any) => {
  ...
  return (
    <div className="grid grid-cols-3 gap-5 gap-y-14">
      {dataResults?.length && checkedList?.length && checkedList2?.length ? (
        dataResults
          .filter(
            (item: any) =>
        ...
        dataResults
          .filter((item: any) => checkedList.includes(item.foodCategory))
          .map((item: any) => {
            return (
              <div
                key={item.id}
        ...
        dataResults
          .filter((item: any) => checkedList2.includes(item.cookingTime))
          .map((item: any) => {
            return (
              <div
                key={item.id}
